...
.env:
	@if $(call IS_GIT_IGNORED); then heroku auth:whoami &>/dev/null || (echo "Please make sure the Heroku CLI is installed and authenticated by running ‘heroku auth:token’.  See more https://toolbelt.heroku.com/. If this is not an app, delete .env from .gitignore." && rm .env && exit 1); fi
